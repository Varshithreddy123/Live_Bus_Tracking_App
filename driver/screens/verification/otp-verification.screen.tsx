...
if (response.data && response.data.user) {
  // Save driver data to AsyncStorage for drivers
  try {
    await AsyncStorage.setItem("driver", JSON.stringify(response.data.user));
    console.log("Driver data saved to AsyncStorage:", response.data.user);
  } catch (storageError) {
    console.error("Failed to save driver data to AsyncStorage:", storageError);
  }

  // Check if driver is new or existing
  if (response.data.isNewUser) {
    // New driver needs to complete profile and bus registration
    router.push({
      pathname: "/(routes)/profile",
      params: { driverId: response.data.user.id }
    });
  } else {
